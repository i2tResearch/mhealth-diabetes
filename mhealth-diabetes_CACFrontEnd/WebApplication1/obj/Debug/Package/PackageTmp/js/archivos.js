...
$.ajax({
    ...
    if (json.length > 0) {
        for (var i = 0 ; i < json.length ; i++) {
            if (json.UrlArchivo == "NO_URL") {
                json.UrlArchivo = "Error";
            }
            var html_fila = "<tr><td><center><a href='#' id='" + json[i].Id + "'><img src='images/excel.png' height='32' width='32'/></a></center><p><center><b>" + json[i].Nombre + "</b></center></p></td><td>" + json[i].NumFilasImportadas + "</td><td>" + json[i].FechaCreacion + "</td><td>" + (json[i].Tamano / 1000) + " KB</td><td><a href='" + json[i].UrlArchivo + "' target=\"_blank\">DESCARGAR<a></td></tr>";
